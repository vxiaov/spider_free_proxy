...
def decode_ss_uri(ss_uri):
    ...
    if ss_uri.startswith('ss://'):
        ...
        if len(netloc) > 1:
            method, password = base64.b64decode(
                b64pading(netloc[0])).decode().split(':', maxsplit=1)
...
def decode_ssr_uri(ssr_uri_string):
    ...
    if len(ssr_split_array) == 2:
        ...
        ssr_decode_string = base64.b64decode(
            b64pading(ssr_encode_string)).decode()
    ...
    server_ip, server_port, protocol, method, obfs, password = server_info[0], server_info[1], server_info[
        2], server_info[3], server_info[4], base64.b64decode(b64pading(server_info[5])).decode()
    server_port = int(server_port)
    # 参数设置
    for i in ['obfs_param', 'protocol_param']:
        if i in ssr_params:
            conf_json[i] = base64.b64decode(
                b64pading(ssr_params[i][0])).decode()
        else:
            conf_json[i] = ""
    conf_json['group'] = 'ssr'
...
def encode_ssr_uri(data):
    ...
    ssr_uri += '/?obfs_param=' + b64encode(data.get('obfs_param', ""))
    ssr_uri += '&protocol_param=' + b64encode(data.get('obfs_param', ""))
    ssr_uri += '&remarks=' + b64encode(data.get('remarks', "SSR"))
    ssr_uri += '&group=' + b64encode("free_ssr")
...
if __name__ == '__main__':
    ...
    hints = '编码URI信息, 两个参数：代理URI类型 代理字符串, 代理URI类型:ss/ssr/v2ray'
    parser.add_argument('-e', '--encode', nargs=2, help=hints)
    parser.add_argument('-d', '--decode', default=None, help='解码URI信息')
    parser.add_argument('-l', '--line', action="store_false", help='解码信息按行输出')
